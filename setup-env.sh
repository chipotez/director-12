...
${KVM_SERVER}  classroom
EOF

qemu-img create -f qcow2 undercloud.qcow2 60G
virt-resize --expand /dev/sda1 ${IMAGE_NAME} undercloud.qcow2
virt-customize -a undercloud.qcow2 --hostname undercloud.example.com --root-password password:${PASSWORD_FOR_VMS} --uninstall cloud-init \
  --selinux-relabel \
